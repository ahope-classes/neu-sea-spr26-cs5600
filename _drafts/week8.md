...
Write pseudo-code to convert a 32-bit virtual address to a 32-bit physical address for a two-level address translation scheme using segmentation at the first level of translation and paging at the second level. Explicitly define whatever constants and data structures you need (e.g. the format of the page table entry, the page size, etc). 

---

Consider a paging system that uses a one-level page table with a virtual memory size of 2^V bytes, a physical memory size of 2^M bytes, and a page size of 2^P bytes. Assume taht the machine has byte addressing and that the entire page table resides in the main memory. The paging scheme is diagrammed in the figure below, and the virtual address as the format:

* high bits: virtual page number
* low bits: offset in page

Part A: How many pages can be stored in physical memory at once? 

Part B: How many entries are in the page table? 

Part C: Assume that V=24, M=21, P=10. How many bits are necessary per page table entry? (Assume that the page table entry contains the physical page number, a "valid" bit, and a "dirty" bit). 

Part D: With the variable values specified in part C, approximately how many pages does the page table require? 

Part E: If P = 10, what is the physical address corresponding to the decimal virtual address 4980? (hint: 2^10 = 1024).  TODO(ahs): add the page table from p. 510 in Computation Structures)
